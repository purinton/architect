...
export default async function ({ mcpServer, toolName, log, discord }) {
  mcpServer.tool(
    ...
    async (_args, _extra) => {
      ...
      if (method === 'create') {
        if (!emojiSettings?.name || !emojiSettings?.image) throw new Error('name and image required for create.');
        ...
        if (!emojiId) throw new Error('emojiId required for delete.');
        const emoji = guild.emojis.cache.get(emojiId);
        if (!emoji) throw new Error('Emoji not found.');
        await emoji.delete();
        return buildResponse({ deleted: true, id: emojiId });
      } else if (method === 'get') {
        if (!emojiId) throw new Error('emojiId required for get.');
        const emoji = guild.emojis.cache.get(emojiId);
        if (!emoji) throw new Error('Emoji not found.');
